                ...
                if subevent == EVT_LE_CONN_COMPLETE:
                    ...
                    for i in range(0, num_reports):
                        if (DEBUG == True):
                            ...
                            logging.debug("\tUDID: ", printpacket(pkt[report_pkt_offset -22: report_pkt_offset - 6]))
                            logging.debug("\tMAJOR: ", printpacket(pkt[report_pkt_offset -6: report_pkt_offset - 4]))
                            logging.debug("\tMINOR: ", printpacket(pkt[report_pkt_offset -4: report_pkt_offset - 2]))
                            ...
                            logging.debug("\t(Unknown):", txpower)
                            rssi, = struct.unpack("b", bytes([pkt[report_pkt_offset -1]]))
                            logging.debug("\tRSSI:", rssi)
